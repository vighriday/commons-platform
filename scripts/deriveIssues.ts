...
import { cellByKey, PLANTED_CELLS } from "../seed/plusCodes.ts";
import { severityNorm, severityLabel } from "../seed/severityTable.ts";
import {
  computeImpact, computeAttention, computeQuadrant, deriveConfidence,
  meanPairwiseCosine, finalRank, isReversal,
} from "../shared/scoring.ts";
import type {
  Report, Issue, IssueType, Category, AgentHandoff, ExposureFactor, VulnerabilityFactor,
...
const expGrid = JSON.parse(readFileSync(path.join(SEED, "exposureGrid.json"), "utf8")) as Record<string, unknown>[];
const vulGrid = JSON.parse(readFileSync(path.join(SEED, "vulnerabilityGrid.json"), "utf8")) as Record<string, unknown>[];
...
const maxEngagement = Math.max(...reports.map((r) => r.engagement.upvotes + 2 * r.engagement.replies), 1);
const times = reports.map((r) => new Date(r.createdAt).getTime());
const minT = Math.min(...times), maxT = Math.max(...times);
const recency = (iso: string) => (maxT === minT ? 1 : (new Date(iso).getTime() - minT) / (maxT - minT));
...
const PLANTED: PlantedDef[] = [
  { id: "ISS_HC1", type: "hidden_crisis", title: "Stormwater drain choke near Agara Lake — silent flood risk", cellKey: "hiddenCrisis1", category: "drainage", severityRow: 5, reportIds: ["R033", "R077"] },
  { id: "ISS_HC2", type: "hidden_crisis", title: "Load-bearing wall crack, Somasundarapalya tenement block", cellKey: "hiddenCrisis2", category: "structural", severityRow: 5, reportIds: ["R064"] },
  { id: "ISS_HC3", type: "hidden_crisis", title: "Streetlight blackout on ORR service road — pedestrian risk", cellKey: "hiddenCrisis3", category: "streetlights", severityRow: 4, reportIds: ["R049", "R069"] },
  { id: "ISS_SYN", type: "synthesis", title: "Failing trunk water main under 27th Main, Sector 1 — pre-rupture", cellKey: "synthesis", category: "water", severityRow: 5, reportIds: ["R041", "R058", "R072", "R086"] },
  { id: "ISS_REC", type: "recurrence", title: "Recurring Drainage Infrastructure Failure — 14th Main x 1st Cross", cellKey: "recurrence", category: "drainage", severityRow: 4, reportIds: ["R028", "R055", "R083"] },
  { id: "ISS_NOISE", type: "noise", title: "Pothole on 17th Cross, Sector 7", cellKey: "noise", category: "roads", severityRow: 2, reportIds: ["R047", "R051", "R052", "R060", "R068", "R074", "R081"] },
];
...
function buildIssue(def: PlantedDef): Issue {
  ...
  const attentionScore = computeAttention({ alarmIntensityMean: alarmMean, upvotes, replies, maxWardEngagement: maxEngagement, recencyNorm });
  ...
  const conf = deriveConfidence({ contributingCount: members.length, adminLevel: "district", meanPairwiseCosine: mpc });
  ...
  const exposure: ExposureFactor = {
    value: exposureVal, source: "open_buildings", provenance: "derived-from-real",
    inputs: { densityNorm: exp.densityNorm as number, heightNorm: exp.heightNorm as number, changeNorm: exp.changeNorm as number },
  };
  const vulnerability: VulnerabilityFactor = {
    value: vulnVal, source: "data_commons", adminLevel: "district", lowGranularityWarning: true, provenance: "real",
    inputs: { deprivationNorm: vul.deprivationNorm as number, floodProneFlag: vul.floodProneFlag as number },
  };

  return {
    issueId: def.id, wardId: WARD, type: def.type, title: def.title, plusCellId: cell.plusCellId,
    contributingReports: def.reportIds, category: def.category, handoff,
    severity: { row: def.severityRow, label: severityLabel(def.category, def.severityRow), norm: sevNorm },
    exposure, vulnerability, impactScore, attentionScore, quadrant,
    recurrence: def.type === "recurrence"
      ? { count: members.length, spanDays: Math.round((new Date(members[members.length - 1].createdAt).getTime() - new Date(members[0].createdAt).getTime()) / 86400000) }
      : null,
    reversal: null, // filled after ranking
    resolution: null, escalation: null, memory: null, // Phase 2 (agents)
...
function assert(c: boolean, m: string) { if (!c) { console.error(`[deriveIssues] FAIL: ${m}`); process.exit(1); } }
const top = finalRank(issues)[0];
assert(top.issueId === "ISS_HC1", `expected HC-1 #1, got ${top.issueId} (impact ${top.impactScore})`);
...
assert(issues.find((i) => i.issueId === "ISS_HC1")!.reversal?.overruledAttention === true, "HC-1 reversal not set");
...
finalRank(issues).forEach((i, n) => console.log(`  #${n + 1} ${i.issueId.padEnd(10)} impact ${String(i.impactScore).padStart(3)} att ${i.attentionScore.toFixed(2)} ${i.quadrant}${i.reversal ? " ⟲REVERSAL" : ""}`));
